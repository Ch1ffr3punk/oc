...
func savePlainTextMessage(data []byte) {
	...
	if timestamp {
		// Einfache Lösung die auf allen Plattformen funktioniert
		epoch := time.Unix(0, 0).UTC()
		if err := os.Chtimes(fullPath, epoch, epoch); err != nil {
			log.Printf("Warning: Could not set timestamp: %v", err)
			log.Printf("Message saved as: %s", fullPath)
		} else {
			log.Printf("Message saved as: %s (timestamp set to 1.1.1970 UTC +0000)", fullPath)
		}
	} else {
		log.Printf("Message saved as: %s", fullPath)
	}
